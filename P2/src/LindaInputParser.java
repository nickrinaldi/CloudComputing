...
public class LindaInputParser {
	...
	private static final String DELETE_HOST_REGEX = "\\w+";
	...
	public static String[] getHostNames(String command) {
		int index = command.indexOf("(");
		if (index == -1 || command.indexOf(")") == -1) {
			return new String[0];
		}
		
		String hosts = command.substring(index);
		ArrayList<String> hostNames = new ArrayList<String>();
		Matcher matcher = Pattern.compile(DELETE_HOST_REGEX).matcher(hosts);

		while (matcher.find()) {
			hostNames.add(matcher.group());
		}
